...
class MYPDF extends TCPDF {
	...
	public function ReadData($result){
		...
		if (mysqli_num_rows($result) > 0) {
			foreach ($result as $cell){
				$data[] = array(
				$cell["CO"],$cell["SR"],$cell["Beg_PM"],$cell["End_PM"],$cell["Location_Description"],$cell["EA"],$cell["Miles"],$cell["Status"],$cell["FO/CC"],$cell["Hubs"],$cell["Plans"],$cell["Project_Status"],$cell["Installation_Year"]
				);
			}
		}
		//echo '<pre>'; print_r($data); echo '</pre>';
	...
	public function ColoredTable($header,$data) {
		...
		$this->SetFont('', 'B');
		// Header width
		$w = array(9,9,15,15,100,16,11,19,11,10,13,20,21);
		...
		foreach($data as $row) {
			...
			$this->Cell($w[12], $h, $row[12], 'LR', 0, 'C', $fill);
			$this->Ln();
			$fill=!$fill;
		}
...
$pdf->SetTitle('D10 ITS FO&CC');
$pdf->SetSubject('D10 ITS Operations');
$pdf->SetKeywords('PDF, District10, Caltrans, ITS Operations, Elements, Inventory');
...
// column titles
$header = array( 'CO', 'RTE', 'BEG_PM', 'END_PM', 'LOCATION', 'EA','MILES', 'STATUS', 'FO/CC','HUBS','PLANS','PJ_STATUS','INSTALL_YR');
